...
## ステータス
本番公開中（[ippo.kanemichi.com](https://ippo.kanemichi.com/)）。バックエンド（AWS EC2 + RDS MySQL）・フロントエンド（Vercel）ともに稼働中。

## 試用
公開サイトから新規登録してすぐに試せます（メールアドレスとパスワードのみ）。アカウント削除（退会）も設定画面から行えます。

## 技術スタック
| レイヤー | 採用技術 |
|---|---|
| バックエンド | Python 3.12 + Django 6 + Django REST Framework |
| 認証 | JWT（`djangorestframework-simplejwt`）を httpOnly Cookie で運用 |
