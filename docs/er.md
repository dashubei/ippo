...
- `anxiety_before` / `anxiety_after` の **0–100 範囲は入力バリデーションで担保**する。**現在未実装（TODO）**：`ExposureRecordSerializer` に `MinValueValidator(0)` / `MaxValueValidator(100)` を追加予定。
...
- **現在未実装（TODO）**：マイグレーションに `Meta.indexes` として追加予定。
